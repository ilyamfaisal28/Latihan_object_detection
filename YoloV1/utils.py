...
def plot_images(image, boxes):
    ...
    for box in boxes:
        box = box[2:]
        assert len(box) == 4, 'Didapatkan nilai yang melebihi x, y, w, h, di box!'
...
def get_bboxes(
        ...
        pred_format='cells',
        box_format='midpoint',
        device='mps', #for NVIDIA, use cuda
):
    all_pred_boxes = []
    all_true_boxes = []
...
def convert_cellboxes(predictions, S=7):
    ...
    convert_bboxes = torch.cat((x, y, w_y), dim=-1)
    predicted_class = predictions[..., :20].argmax(-1).unsqueeze(-1)
    best_confidence = torch.max(predictions[..., 20], predictions[..., 25]).unsqueeze(-1)
    converted_preds = torch.cat(
        (predicted_class, best_confidence, convert_bboxes), dim=-1
    )
